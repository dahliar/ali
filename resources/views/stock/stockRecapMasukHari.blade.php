...
<script type="text/javascript">
    ...
    function barcodeList(storageDate, itemId){
        window.open(('{{ url("scanRekapMasukHariBarcodeList") }}'+"/"+storageDate+"/"+itemId), '_self');
    }

    $(document).ready(function() {
        myFunction(document.getElementById("tanggal").value);

    });
    function myFunction(storageDate){
        $('#datatable').DataTable({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            ajax:{
                url: '{{ url("getScanMasukHarianTanggal") }}',
                data: function (d){
            ...
            <div class="modal-header">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb primary-color my-auto">
                        ...
                        <li class="breadcrumb-item active">Tanggal {{$storageDate}}</li>
                    </ol>
                </nav>
            </div>
        </div>
