...
<script type="text/javascript">
    ...
    function tambahIPL(id){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: ('{{ url("purchase/notaPembelian") }}'+"/"+id),
                    type: "GET",
                    dataType: "json",
    ...
    function myFunction(purchaseId){
        $('#datatable').DataTable({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            ajax:{
                url: '{{ url("getAllPurchaseDocuments") }}',
                data: function (d){
            ...
            <div class="modal-header">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb primary-color my-auto">
                        ...
                        <li class="breadcrumb-item active">Dokumen Transaksi Pembelian</li>
                    </ol>
                </nav>
                <div>
                    @if ($purchase->status==2)
                    <button onclick="tambahIPL({{$purchase->id}})" class="btn btn-primary" data-toggle="tooltip" data-placement="top" data-container="body" title="Cetak Invoice Baru">
