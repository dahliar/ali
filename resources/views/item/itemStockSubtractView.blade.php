...
<script type="text/javascript">
    ...
    function editStoreDetail(id){
        window.open(('{{ url("itemStockSubtractEdit") }}' + "/"+ id), '_self');
    }
    
    function deleteStoreDetail(id){
        Swal.fire({
            title: 'Hapus pengurangan stock',
            text: "Hapus?",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ url("deleteStockSubtractChange") }}',
                    type: "POST",
                    data: {
                        "_token":"{{ csrf_token() }}",
                        subtractId   : id
    ...
    function myFunction(){
        ...
        $('#datatable').DataTable({ 
            ajax:'{{ url("getItemSubtractHistory") }}' + "/"+ itemId+ "/"+ start+ "/"+ end+ "/"+ opsi,
            serverSide: false,
            processing: true,
            deferRender: true,
            type: 'GET',
            ...
            <div class="modal-header">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb primary-color my-auto">
                        ...
                        <li class="breadcrumb-item active">Histori pengurangan stock barang</li>
                    </ol>
                </nav>
            </div>
